...
```{r}
peru <- peru_raw %>%
select(wvsi2, b6, anestg, pn4, exc6, exc7new)
```
...
```{r}
pltree(agnes_res, main = "Dendrograma AGNES", cex = 0.6)
```
```{r}
fviz_dend(
  as.hclust(agnes_res),
...
fviz_dend(
  as.hclust(diana_res),
  k = 2,              # número de clusters que quieres resaltar
  rect = TRUE,        # dibuja rectángulos alrededor de los clusters
  show_labels = FALSE,
